# Task: Fix 406 Not Acceptable from LEGO MPS

## Context
`src/openai_service_worldofgeese/__main__.py` in `/home/node/.openclaw/workspace/projects/little_bits_of_buddha`.
The `_call_lego_mps` function sends requests to LEGO MPS (an Anthropic-compatible proxy).
...
## What to do
1. In `_call_lego_mps()`, add `"anthropic-version": "2023-06-01"` to the `headers` dict.
