...
- **Base URL:** `https://ANTHROPIC_PROXY_HOST/claude`
...
**Important:** LiteLLM auto-appends `/v1/messages` to `api_base` for Anthropic provider. Verify that `https://ANTHROPIC_PROXY_HOST/claude/v1/messages` is the correct full endpoint. If LEGO MPS expects a different path, you may need to adjust.
...
- Add `ANTHROPIC_BASE_URL=https://ANTHROPIC_PROXY_HOST/claude` as env var in compose.yaml for the openai-service
