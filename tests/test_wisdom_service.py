...
class TestWisdomAsk:
    ...
    def test_system_prompt_changes_with_practice_level_newcomer(
        ...
        mock_rag.return_value = [{"role": "system", "content": "..."}, {"role": "user", "content": "test"}]
        mock_proxy.return_value = mock_anthropic_response
        ...
        assert "encountering the Dhamma for the first time" in call_args.kwargs["system_prompt"]
    ...
    def test_sutta_context_injected_by_rag(
        self, mock_proxy, mock_search, client, mock_sutta_results, mock_anthropic_response
    ):
    ...
    def test_sutta_citation_extraction(self, client):
        ...
        text = "As taught in SN56.11 and MN10, the path leads to DN22. See also AN4.170."
    ...
    def test_theme_detection_from_user_message(self, client):
        ...
        themes = _extract_themes("What is the relationship between suffering, impermanence, and non-self?")
        assert "suffering" in themes
    ...
    def test_fallback_to_raw_httpx_when_conversation_api_fails(
        self, mock_rag, mock_proxy, mock_conversation_api, client, mock_anthropic_response
    ):
